...
class ParentFeesScreen extends StatefulWidget {
  ...
  const ParentFeesScreen({super.key, required this.userData});
...
class _ParentFeesScreenState extends State<ParentFeesScreen> {
  ...
  void initState() {
    super.initState();
    _fetchWardFeeSummary();
  }

  Future<void> _fetchWardFeeSummary() async {
    setState(() => _isLoading = true);
    final userId = widget.userData['id']?.toString() ?? widget.userData['login_id']?.toString() ?? '';
    try {
      final result = await ApiConfig.get(
        '${ApiConstants.baseUrl}/api/finance/parent-summary?userId=$userId',
      );
      ...
      debugPrint("Error fetching parent ward fees summary: $e");
      setState(() => _isLoading = false);
    }
  }
  ...
  void _showWhyMyFeeDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text("Why Is My Ward's Fee This Amount?"),
        content: Column(
  ...
  Widget build(BuildContext context) {
    ...
    if (_ledger == null) {
      return Scaffold(
        appBar: AppBar(title: const Text("Ward's Fees")),
        body: const Center(child: Text("Child fees not configured or linked in portal.")),
      );
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("${_ledger!['studentName']}'s Fee Ledger"),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // Summary Card (Parent read-only summary card)
            Card(
              elevation: 4,
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Current Fee Ledger Summary (Parent Portal)", style: TextStyle(color: isDark ? Colors.amber : Colors.orange, fontWeight: FontWeight.bold)),
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton(
                            ...
                            child: const Text("Why this amount?", style: TextStyle(color: Colors.orange)),
                          ),
                        ),
                      ],
                    ),
            ...
            changes.isEmpty
                ? const Text("No adjustments made to child base structures.", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic))
                : ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("₹${c['previousAmount'].toStringAsFixed(0)} ➔ ₹${c['newAmount'].toStringAsFixed(0)}", style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
                              const SizedBox(height: 4),
