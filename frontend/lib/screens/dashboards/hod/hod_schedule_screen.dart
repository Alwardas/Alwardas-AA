...
class _HodScheduleScreenState extends State<HodScheduleScreen> {
  ...
  void _onSlotTap(Map<String, dynamic> item) async {
    ...
    await _openAssignClassScreen(item);
  }
  ...
  Future<void> _openAssignClassScreen(Map<String, dynamic> item) async {
      final updated = await Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => HodAssignClassScreen(
                  branch: item['branch'] ?? _userBranch ?? "Computer Engineering",
                  year: item['year'] ?? "1st Year",
                  section: item['section'] ?? "A",
                  initialDay: _selectedDay,
                  initialPeriod: item['number'],
                  initialData: item['subject'] != '---' && item['subject'] != null ? {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _openAssignClassScreen({}),
