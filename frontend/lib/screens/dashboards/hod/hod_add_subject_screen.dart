...
class HodAddSubjectScreen extends StatefulWidget {
  const HodAddSubjectScreen({super.key});
...
class _HodAddSubjectScreenState extends State<HodAddSubjectScreen> {
  ...
  Future<void> _fetchSections() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("${ApiConstants.hodGetSections}?branch=$_selectedBranch&year=$_selectedYear"),
      );
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> _fetchSubjects() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("${ApiConstants.hodGetSubjects}?branch=$_selectedBranch&year=$_selectedYear"),
      );
      if (response.statusCode == 200) {
        setState(() {
  ...
  Widget _buildHeader(Color tint, Color textColor, Color subTextColor) {
    return Column(
      ...
      children: [
        ...
        Text(
          "Configure subjects for different branches and sections.",
          style: GoogleFonts.poppins(
            fontSize: 14,
            color: subTextColor,
          ),
            ...
            Expanded(
              child: loading
                  ...
                  : Text(
                      value ?? hint,
                      style: GoogleFonts.poppins(
                        color: value != null ? textColor : textColor.withValues(alpha: 0.5),
                        fontSize: 14,
                      ),
                    ),
            ),
