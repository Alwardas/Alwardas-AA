...
pub async fn get_timetable_handler(
    ...
    if let Some(fid) = params.get("facultyId") {
        return sqlx::query_as::<Postgres, TimetableEntry>("SELECT t.*, u.full_name as faculty_name, u.email as faculty_email, u.phone_number as faculty_phone, u.branch as faculty_department FROM timetable_entries t LEFT JOIN users u ON t.faculty_id = u.login_id WHERE t.faculty_id = $1")
            .bind(fid)
            .fetch_all(&state.pool)
            .await
            .map_err(|e| {
    ...
    if let (Some(branch), Some(year)) = (params.get("branch"), params.get("year")) {
        ...
        return sqlx::query_as::<Postgres, TimetableEntry>("SELECT t.*, u.full_name as faculty_name, u.email as faculty_email, u.phone_number as faculty_phone, u.branch as faculty_department FROM timetable_entries t LEFT JOIN users u ON t.faculty_id = u.login_id WHERE t.branch = $1 AND t.year = $2 AND t.section = $3 ORDER BY t.day, t.period_index")
