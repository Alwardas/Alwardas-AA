...
pub async fn assign_lesson_schedule(pool: &PgPool, payload: AssignLessonScheduleRequest) -> Result<(), StatusCode> {
    ...
    faculty_repository::insert_lesson_schedule(&mut tx, &payload.subject_id, &payload.topic_id, &payload.schedule_date, &payload.branch, &payload.year, &payload.section)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
